/**
 *  @file   ExampleAlgorithms/LArFormattingHelper.cc
 *
...
{
    LArFormattingHelper::SetStyle(REGUEXAMPLE, stream);
}
...
{
    for (unsigned int i = 0, nColumns = m_columnTitles.size(); i < nColumns; ++i)
        this->PrintTableCell(std::string(m_widths[i], '-'), LArFormattingHelper::GetFormatCharacter(LArFormattingHelper::REGUEXAMPLE), i);
}
